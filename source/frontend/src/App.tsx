...
function App() {
    ...
    const loadState = async () => {
        try {
            ...
            if (data.mode === 'workspace') {
                const settings = await fetch(`/api/settings?t=${Date.now()}`).then((res) => res.json());
    ...
    const downloadSourceFile = async () => {
        try {
            const response = await fetch(`/api/file?path=.aicoder/source.txt&t=${Date.now()}`);
    ...
    return (
        <>
            ...
            {mode === 'workspace' && (
                <div className="workspace-shell fixed inset-0 flex h-full w-full flex-col overflow-hidden bg-background font-sans text-foreground">
                    <header className="compact-header app-header border-b border-border/60">
                        ...
                        <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
                            <Button 
                                ...
                                className="emerald-pill h-9 gap-2 rounded-full border px-3 text-xs font-mono hover:bg-emerald-500/20 dark:hover:text-white"
                            ...
                            <Button
                                variant="ghost"
                                size="sm"
                                onClick={openPromptTemplate}
                                className="h-9 gap-2 rounded-full px-3 text-xs text-muted-foreground hover:text-foreground"
                                title={t('app.prompt_template')}
                            >
                    ...
                    <ResizablePanelGroup direction="horizontal" className="flex-1 overflow-hidden">
                        ...
                        <ResizableHandle className="w-[1px] bg-border/40 transition-colors hover:bg-primary/50" />

                        <ResizablePanel defaultSize={58} className="workspace-main bg-background">
                            {currentFile ? <CodeEditor /> : <ChatInterface />}
                        </ResizablePanel>
